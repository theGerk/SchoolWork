...
Here we prove some lemmas that we will use while proving Kuratowski's theorem before getting into the meat of the theorem.
...
This means that we may freely interchange usage of a sphere with a plane; for our purposes they are the same.

\begin{theorem}[Euler's equation for planar graphs]
	Let the number of edges in the graph be $e$, the number of verticies by $v$ and the number of faces be $f$.
\end{theorem}
